...
df_visits = pd.read_excel('FileName.xlsx') #FileName.xlsx is a generic file place holder.
...
df_visits_cln = new_df_visits.dropna().loc[new_df_visits['Item'] != 50]

#show_me = df_visits_cln.iloc[1:15]
#print(show_me)
#column_dtype = df_visits_cln['StartDate'].dtype
#print(column_dtype)
...
#calculate minutes over/under visit allowance; 0.50 represents the expected visit duration
ave_result['Over_Under'] = (ave_result['visit_time']) - 0.50
...
ave_result['Corrected_price'] = (ave_result['ExtPrice'] / 38) * (ave_result['visit_time'] * 60)
...
ave_result.to_excel('outputFileName.xlsx', index=False)
